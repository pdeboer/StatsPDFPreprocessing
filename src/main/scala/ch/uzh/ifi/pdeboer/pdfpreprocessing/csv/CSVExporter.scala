package ch.uzh.ifi.pdeboer.pdfpreprocessing.csv

import java.io.File

import ch.uzh.ifi.pdeboer.pdfpreprocessing.png.StatTermLocationsInPNG
...
class CSVExporter(file: String, snippets: Iterable[Snippet]) {
	def persist() {
		...
		snippets.foreach(p => writer.writeRow(Seq(p.groupName, p.methodIdentifier, p.snippetPath, p.permutation.paper.file.getAbsolutePath, p.methodOnTop, p.statTermLocationsInPNG.relativeTop, p.statTermLocationsInPNG.relativeBottom)))
		writer.close()
	}
}
